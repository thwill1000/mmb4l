Option Base 0
Option Default None
Option Explicit

If Mm.Device$ = "MMB4L" Then Option Simulate "PicoMiteVGAUSB"

Dim cmd$, id%
Dim gamepad_present%(4)
Dim gamepad_interrupt%(4)
Dim gamepad_usb$(4, 2) Length 10

Console Clear
Cls

For id% = 3 To 4
  cmd$ = "Device Gamepad Interrupt Enable id%, on_gamepad" + Str$(id%)
  On Error Skip
  Execute cmd$
  If Not Mm.ErrNo Then gamepad_present%(id%) = 1
  gamepad_usb$(id%, 0) = Str$(Mm.Info(Usb id%))
  gamepad_usb$(id%, 1) = Hex$(Mm.Info(Usb Vid id%))
  gamepad_usb$(id%, 2) = Hex$(Mm.Info(Usb Pid id%))
  print_gamepad(id%)
Next

Print
Print
Print "A - Start rumble, B - Stop rumble, X - Interrupt cursors, Left - Interrupt all"

Do
  Console HideCursor
  For id% = 3 To 4
    update_gamepad(id%)
  Next
  Pause 100
  Console ShowCursor
Loop

Sub print_gamepad(id%)
  Const y% = (id% - 3) * 9
  Print @(0, y%) "Gamepad " + Str$(id%) + " - " + Choice(gamepad_present%(id%), "PRESENT", "NOT PRESENT")
  Print "USB Device: " + gamepad_usb$(id%, 0) + ", VID: &h" + gamepad_usb$(id%, 1) + ", PID: &h" + gamepad_usb$(id%, 2)
  Print "Buttons:                  ########"
  Print "Left  Analog Button:      ########"
  Print "Right Analog Button:      ########"
  Print "Left  Analog Stick:   x = ########, y = ########"
  Print "Right Analog Stick:   x = ########, y = ########"
End Sub

Sub update_gamepad(id%)
  If Not gamepad_present%(id%) Then Exit Sub

  Const y% = (id% - 3) * 9

  Const btn% = Device(Gamepad id%, B)
  Const b$ = str.rpad$(buttons_as_string$(btn%), 40)
  Const l$ = str.lpad$(Str$(Device(Gamepad id%, L)), 8)
  Const lx$ = str.lpad$(Str$(Device(Gamepad id%, LX)), 8)
  Const ly$ = str.lpad$(Str$(Device(Gamepad id%, LY)), 8)
  Const r$ = str.lpad$(Str$(Device(Gamepad id%, R)), 8)
  Const rx$ = str.lpad$(Str$(Device(Gamepad id%, RX)), 8)
  Const ry$ = str.lpad$(Str$(Device(Gamepad id%, RY)), 8)

  Print @(22, y% + 2) b$
  Print @(26, y% + 3) l$
  Print @(26, y% + 4) r$
  Print @(26, y% + 5) lx$
  Print @(40, y% + 5) ly$
  Print @(26, y% + 6) rx$
  Print @(40, y% + 6) ry$
  Print @(0, y% + 7) Choice(gamepad_interrupt%(id%) > 0, "*INTERRUPT*", "           ")
  If gamepad_interrupt%(id%) > 0 Then Inc gamepad_interrupt%(id%), -1

  Local cmd$
  Select Case btn%
    Case &h100 ' Left
      'Device Gamepad Close id%
      'cmd$ = "Device Gamepad Open id%, on_gamepad" + Str$(id%)
      cmd$ = "Device Gamepad Interrupt Enable id%, on_gamepad" + Str$(id%)
      On Error Skip
      Execute cmd$
    Case &h400 ' X
      'Device Gamepad Close id%
      'cmd$ = "Device Gamepad Open id%, on_gamepad" + Str$(id%) + ", &b0000000111100000"
      cmd$ = "Device Gamepad Interrupt Enable id%, on_gamepad" + Str$(id%) + ", &b0000000111100000"
      On Error Skip
      Execute cmd$
    Case &h800 ' A
      Device Gamepad Haptic id%, &hFF, &hFF
    Case &h2000 ' B
      Device Gamepad Haptic id%, 0, 0
  End Select
End Sub

Function str.lpad$(s$, x%)
  str.lpad$ = s$
  If Len(s$) < x% Then str.lpad$ = Space$(x% - Len(s$)) + s$
End Function

Function str.rpad$(s$, x%)
  str.rpad$ = s$
  If Len(s$) < x% Then str.rpad$ = s$ + Space$(x% - Len(s$))
End Function

Function buttons_as_string$(btn%)
  Local s$
  If btn% And &h01 Then Cat s$, "R, "
  If btn% And &h02 Then Cat s$, "Start, "
  If btn% And &h04 Then Cat s$, "Home, "
  If btn% And &h08 Then Cat s$, "Select, "
  If btn% And &h10 Then Cat s$, "L, "
  If btn% And &h20 Then Cat s$, "Down, "
  If btn% And &h40 Then Cat s$, "Right, "
  If btn% And &h80 Then Cat s$, "Up, "
  If btn% And &h100 Then Cat s$, "Left, "
  If btn% And &h200 Then Cat s$, "ZR, "
  If btn% And &h400 Then Cat s$, "X, "
  If btn% And &h800 Then Cat s$, "A, "
  If btn% And &h1000 Then Cat s$, "Y, "
  If btn% And &h2000 Then Cat s$, "B, "
  If btn% And &h4000 Then Cat s$, "ZL, "
  If Len(s$) Then s$ = Left$(s$, Len(s$) - 2)
  buttons_as_string$ = s$
End Function

Sub on_gamepad(id%)
  gamepad_interrupt%(id%) = 20
End Sub

Sub on_gamepad3()
  on_gamepad(3)
End Sub

Sub on_gamepad4()
  on_gamepad(4)
End Sub
