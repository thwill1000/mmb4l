...
MmResult gamepad_analog(int32_t sdlId, uint8_t sdlAxis, int16_t value) { return kError; }
MmResult gamepad_button_down(int32_t sdlId, uint8_t sdlButton) { return kError; }
MmResult gamepad_button_up(int32_t sdlId, uint8_t sdlButton) { return kError; }
...
class SpriteTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        // Create 10 active sprites 10x10 at (x,y) = (20, 20), (40, 40), (60, 60), ...
        for (MmSurfaceId id = 1; id <= 10; ++id) {
            (void) graphics_sprite_create(id, 10, 10);
...
TEST_F(SpriteTest, Create) {
    ...
    EXPECT_EQ(0, sprite->x);
    EXPECT_EQ(0, sprite->y);
    EXPECT_EQ(0, sprite->layer);
...
TEST_F(SpriteTest, InactiveSpritesDoNotCollideWithSprites) {
    MmSurface *sprite1 = &graphics_surfaces[1];
    MmSurface *sprite3 = &graphics_surfaces[3];
    EXPECT_EQ(kOk, sprite_activate(sprite1, true));
    EXPECT_EQ(kOk, sprite_activate(sprite3, false));
    GivenSurfacesOverlapping(1, 3);
    EXPECT_EQ(kOk, sprite_update_all_collisions());
...
TEST_F(SpriteTest, InactiveSpritesDoNotCollideWithEdges) {
    MmSurface *sprite1 = &graphics_surfaces[1];
    EXPECT_EQ(kOk, sprite_activate(sprite1, false));
    GivenSurfaceOverlappingEdge(1, kSpriteEdgeLeft);
    EXPECT_EQ(kOk, sprite_update_all_collisions());
...
TEST_F(SpriteTest, SpritesDoNotCollideWithWindows) {
    EXPECT_EQ(kOk, graphics_window_create(20, 0, 0, 10, 10, 1, "title", NULL));
    GivenSurfacesOverlapping(1, 20);
