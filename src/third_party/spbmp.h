...
typedef enum {
    ...
    // 4-bit formats
    kSpBmp4bppRgb121,
    kSpBmp4bppRgb121Rle4,

    // 8-bit formats
    kSpBmp8bppRgb222,
    kSpBmp8bppRgb222Rle8,
    kSpBmp8bppRgb332,
    kSpBmp8bppRgb332Rle8,
...
typedef SpColourArgb (*SpBmpGetPixelCb)(int x, int y, void *userdata);

/** Callback that sets a pixel. */
typedef void (*SpBmpSetPixelCb)(int x, int y, SpColourArgb colour, void *userdata);
 ...
 * @param  abort_check_cb  Callback to check if the BMP read should be aborted
 ...
 */
SpBmpResult spbmp_read(void *file, int x, int y, void *userdata);

/**
 * Writes a bitmap image (.bmp file).
 *
 * @param  file      Opaque pointer to file that should be written.
 * @param  format    Format to use for .bmp file.
 * @param  userdata  Opaque pointer to "user data" that will be sent to callback functions.
 * @param  x         The x-coordinate of the top left corner on the surface of the image to write.
 * @param  y         The y-coordinate of the top left corner on the surface of the image to write.
