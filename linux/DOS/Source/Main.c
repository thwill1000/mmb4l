...
int main(int argc, char* argv[]) {
  ...
  // if there is something on the command line try to load it as a program, if that fails try AUTORUN.BAS
  if(argc > 1) RunCommandLineProgram = LoadFile(argv[1]);
    if(!RunCommandLineProgram) RunCommandLineProgram = LoadFile("AUTORUN.BAS");
    if(!RunCommandLineProgram) RunCommandLineProgram = LoadFile("C:\\AUTORUN.BAS");
  ...
  if(RunCommandLineProgram) {
      ...
      ClearRuntime();
      PrepareProgram(true);
      ExecuteProgram(ProgMemory);                                   // if AUTORUN.BAS or something is on the command line, run it
  }
...
int LoadFile(char *prog) {
    ...
    if(f != NULL) {
        fclose(f);
        buf[0] = '"'; strcpy(&buf[1], prog); strcat(buf, "\"");
        FileLoadProgram(buf);
        if(*ProgMemory == T_NEWLINE || *ProgMemory == T_LINENBR) {  // is there a program to run?
            return true;
        }
    }
    return false;
...
void DOSCursor(int x, int y) {
  ...
  Option.Width = consoleinfo.srWindow.Right - consoleinfo.srWindow.Left;
    ...
    if(coord.X > consoleinfo.srWindow.Right || coord.Y > consoleinfo.srWindow.Bottom) return;
...
int DOSgetch(void) {
    int c;
    char s;
    c = getch();                                                    // get the first character of a possible multibyte function key
...
void MMgetline(int filenbr, char *p) {
    ...
    while(1) {
        ...
        if(c == '\r') {
            if(filenbr == 0) {
                MMPrintString("\r\n");
                break;                                              // on the console this meand the end of the line - stop collecting
            } else
                continue ;                                          // for files loop around looking for the following newline
        }

        if(isprint(c)) {
            if(filenbr == 0) MMputchar(c);                          // The console requires that chars be echoed
        }
